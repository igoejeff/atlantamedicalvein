# Atlanta Medical Clinic — Website & SEO Documentation

## 🏥 Project Overview
**Live Site:** https://www.atlantamedicalclinic.com/  
**Clinic:** Atlanta Medical Clinic — Board-certified varicose vein & spider vein specialists  
**Address:** 197 14th St NW, Ste 300, Atlanta, GA 30318  
**Phone:** 404-609-0870  
**Email:** info@atlantamedicalclinic.com  
**Hours:** Mon–Fri 8:00am–5:00pm

---

## ✅ Completed Features

### Core Pages
| Page | URL | Status |
|------|-----|--------|
| Homepage | `index.html` | ✅ Complete |
| Blog & Resources | `blog.html` | ✅ Complete |
| Our Team | `team-page.html` | ✅ (source from AMC) |
| Varicose Vein Treatment | `services/varicose-vein-treatment.html` | ✅ SEO-optimized |
| Spider Vein Treatment | `services/spider-vein-treatment.html` | ✅ SEO-optimized |
| VENCLOSE™ RFA | `services/venclose-rfa.html` | ✅ SEO-optimized |
| Varithena® Microfoam | `services/varithena.html` | ✅ SEO-optimized |
| Sclerotherapy & ClariVein® | `services/sclerotherapy.html` | ✅ SEO-optimized |

### Technical SEO Files
| File | Purpose |
|------|---------|
| `sitemap.xml` | XML sitemap with image extensions — submit to Google Search Console |
| `robots.txt` | Crawler instructions with sitemap pointer |

---

## 🔍 SEO Implementation — Gold Standard

### 1. Primary Meta Tags (All Pages)
- ✅ Unique, keyword-rich `<title>` tags (50–60 chars)
- ✅ Unique `<meta name="description">` (140–160 chars with primary keywords)
- ✅ `<meta name="keywords">` with geo-targeted terms
- ✅ `<meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1">`
- ✅ `<meta name="author">` set to Atlanta Medical Clinic
- ✅ Geo meta tags: `geo.region`, `geo.placename`, `geo.position`, `ICBM` (homepage)
- ✅ `<link rel="canonical">` on every page

### 2. Open Graph (Facebook / LinkedIn)
- ✅ `og:type`, `og:locale`, `og:site_name`
- ✅ Unique `og:title`, `og:description`, `og:url` per page
- ✅ `og:image` with width/height/alt attributes
- ✅ Real clinic photography used for social sharing previews

### 3. Twitter Card
- ✅ `twitter:card` = `summary_large_image`
- ✅ `twitter:site` = `@ATLMedicalClinic`
- ✅ Unique `twitter:title`, `twitter:description`, `twitter:image` per page

### 4. JSON-LD Structured Data (Schema.org)

#### Homepage (`index.html`) — Full @graph array:
- ✅ **MedicalClinic / MedicalBusiness / LocalBusiness** — with full NAP, geo, hours, services
- ✅ **WebSite** — with SearchAction sitelinks searchbox
- ✅ **WebPage** — with breadcrumb, speakable spec
- ✅ **FAQPage** — 7 questions answering top search queries
- ✅ **AggregateRating** — 4.7 stars, 5000 reviews
- ✅ **3 individual Review** entries with author, date, body
- ✅ **4 MedicalProcedure** entries under availableService
- ✅ sameAs links to Facebook, Instagram, LinkedIn, Yelp, Healthgrades, Vitals, ZocDoc

#### Blog Page (`blog.html`):
- ✅ **Blog** schema with 5 BlogPosting entries (headline, description, image, datePublished, keywords)
- ✅ **BreadcrumbList** — Home > Blog & Resources

#### Service Pages (all 5):
- ✅ **MedicalWebPage** with MedicalProcedure/MedicalCondition
- ✅ **BreadcrumbList** — Home > Services > [Service Name]
- ✅ **FAQPage** with 3–4 service-specific questions each
- ✅ Inline microdata (`itemscope`, `itemprop`) for procedure names, descriptions

#### Footer:
- ✅ Inline microdata on MedicalClinic with PostalAddress, telephone, email, openingHours

### 5. Semantic HTML Structure
- ✅ `<html lang="en" itemscope itemtype="https://schema.org/MedicalClinic">`
- ✅ `<header role="banner">`, `<main id="main-content" role="main">`, `<footer role="contentinfo">`
- ✅ `<nav role="navigation" aria-label="Primary Navigation">`
- ✅ All sections have `aria-labelledby` pointing to unique heading IDs
- ✅ All images have descriptive `alt` text
- ✅ Heading hierarchy: H1 → H2 → H3 (no skips)
- ✅ `aria-current="page"` on active nav items

### 6. Performance (Core Web Vitals)
- ✅ `<link rel="preconnect">` for Google Fonts, fonts.gstatic.com, jsDelivr
- ✅ `<link rel="dns-prefetch">` for Google Analytics, GTM
- ✅ Font display: swap on all Google Fonts
- ✅ `<meta name="theme-color">` for mobile browser chrome

### 7. Internal Linking Architecture
- ✅ Each treatment card on homepage links to its dedicated service page
- ✅ Each service page has a "Related Services" grid linking to all other service pages
- ✅ Blog articles link back to relevant service pages
- ✅ Footer has full services column with links to all 5 service pages
- ✅ Breadcrumb navigation on all service pages and blog

### 8. XML Sitemap (`sitemap.xml`)
- ✅ All core pages with `<lastmod>`, `<changefreq>`, `<priority>`
- ✅ Image extensions (`image:image`, `image:title`, `image:caption`) on main pages
- ✅ All 8 blog post URLs included
- ✅ Priority: Homepage=1.0, Services=0.85–0.9, Blog=0.75–0.9

### 9. robots.txt
- ✅ Permits all major crawlers (Googlebot, Bingbot, Slurp, DuckDuckBot, Baidu, Yandex)
- ✅ Blocks source files and dev artifacts
- ✅ Points to sitemap URL

### 10. Local SEO Signals
- ✅ NAP (Name, Address, Phone) consistent across all pages
- ✅ Geo coordinates in JSON-LD (33.9212, -84.3132) and meta tags
- ✅ "Atlanta, GA" appears naturally in all page titles and descriptions
- ✅ LocalBusiness schema with `hasMap` pointing to Google Maps
- ✅ Schema `openingHoursSpecification` on main organization

---

## 📁 File Structure

```
index.html                          ← Homepage (SEO priority 1.0)
blog.html                           ← Blog hub
team-page.html                      ← Team page (from AMC source)
sitemap.xml                         ← XML sitemap — SUBMIT TO GOOGLE
robots.txt                          ← Crawler rules
README.md                           ← This file

services/
  varicose-vein-treatment.html      ← Target: "varicose vein treatment Atlanta"
  spider-vein-treatment.html        ← Target: "spider vein removal Atlanta"
  venclose-rfa.html                 ← Target: "VENCLOSE RFA Atlanta"
  varithena.html                    ← Target: "Varithena Atlanta"
  sclerotherapy.html                ← Target: "sclerotherapy Atlanta"

css/
  style.css                         ← Main stylesheet

js/
  main.js                           ← Main JavaScript

images/
  amc-logo.png                      ← Clinic logo
  hero-bg.jpg                       ← Hero background
  leg-pain.jpg                      ← Symptoms section
  ba-photo-1.jpg                    ← Before/after #1 (Sonya T.)
  ba-photo-2.jpg                    ← Before/after #2 (Raymond M.)
  ba-photo-3.jpg                    ← Before/after #3 (Leonida V.)
  video-poster.jpg                  ← Video thumbnail
  blog/
    blog-diabetes.jpg
    blog-knee-pain.jpg
    blog-circulation.jpg
    blog-neuropathy.jpg
    blog-knee-risks.jpg
    blog-spider-veins.jpg
    blog-knee-nonsurgical.jpg
    blog-venclose-rfa.jpg
    blog-diabetes-screenings.jpg
  insurance/
    bcbs.png / bcbs.svg
    aetna.png / cigna.svg
    unitedhealthcare.png / humana.png
    medicare.png / medicaid.svg / tricare.png
```

---

## 🚀 Next Steps for Maximum SEO Impact

### Immediate Actions (Do Today)
1. **Submit sitemap to Google Search Console** at https://search.google.com/search-console
   - Go to Sitemaps → Enter: `https://www.atlantamedicalclinic.com/sitemap.xml`
2. **Claim Google Business Profile** at https://business.google.com
   - Add all photos, services, hours — this drives local pack rankings
3. **Request reviews** on Google from your 5,000+ patients

### Short-Term (This Week)
4. **Add Google Analytics 4** tracking code to `<head>` of all pages
5. **Add Google Tag Manager** for conversion tracking (form submissions, phone clicks)
6. **Set up Bing Webmaster Tools** — submit sitemap there too
7. **Create a Google Business Profile post** linking to the new service pages

### Medium-Term (This Month)
8. **Backlinks strategy:**
   - List clinic on Healthgrades, Vitals, WebMD, Zocdoc, RateMDs, US News Health
   - Submit to local Atlanta medical directories
   - Guest post on Atlanta health/lifestyle blogs
   - Partner with referring physicians for reciprocal links
9. **Add FAQ section to homepage** (the JSON-LD FAQ schema is already built — add visible HTML FAQ)
10. **Schema enhancements:**
    - Add `Physician` schema for each doctor
    - Add `MedicalStudy` or `ClinicalTrial` references if applicable
    - Add `EventScheduled` schema for health events or webinars

### Long-Term
11. **Content marketing:** Publish 2 blog posts/month targeting new keywords
12. **Video SEO:** Add YouTube channel with procedure explainer videos; embed in service pages
13. **Voice search optimization:** The FAQ schema already helps — add more conversational Q&As
14. **Page speed:** Run Google PageSpeed Insights and optimize image sizes/formats (WebP)

---

## 🎯 Target Keywords by Page

| Page | Primary Keyword | Secondary Keywords |
|------|----------------|-------------------|
| index.html | varicose vein treatment Atlanta | spider vein Atlanta, vein clinic Atlanta GA |
| varicose-vein-treatment.html | varicose vein treatment Atlanta | varicose vein doctor Atlanta, insurance covered vein treatment |
| spider-vein-treatment.html | spider vein removal Atlanta | sclerotherapy Atlanta, spider vein specialist near me |
| venclose-rfa.html | VENCLOSE RFA Atlanta | radiofrequency ablation Atlanta, RFA vein treatment |
| varithena.html | Varithena Atlanta | polidocanol microfoam Atlanta, foam vein treatment |
| sclerotherapy.html | sclerotherapy Atlanta | ClariVein Atlanta, vein injections Atlanta |
| blog.html | vein health blog Atlanta | varicose vein articles, leg pain education |

---

## 📊 SEO Checklist

- [x] Title tags optimized (50–60 chars)
- [x] Meta descriptions (140–160 chars)
- [x] Canonical URLs on all pages
- [x] Open Graph tags
- [x] Twitter Card tags
- [x] JSON-LD structured data (MedicalClinic, FAQPage, BreadcrumbList, Blog)
- [x] Geo meta tags
- [x] XML sitemap
- [x] robots.txt
- [x] Semantic HTML5 with ARIA roles
- [x] Breadcrumb navigation
- [x] Internal linking (every page links to related pages)
- [x] Alt text on all images
- [x] Preconnect/DNS-prefetch for performance
- [x] Mobile-responsive design
- [x] Footer with NAP schema markup
- [ ] Google Search Console setup
- [ ] Google Business Profile optimization
- [ ] Google Analytics 4 installed
- [ ] Core Web Vitals baseline measured
- [ ] Backlink acquisition started

---

*Last updated: March 2, 2026*
